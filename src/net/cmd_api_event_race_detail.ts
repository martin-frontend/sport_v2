 ...
 * 赛事详情
 */
export default class cmd_api_event_race_detail extends puremvc.SimpleCommand {
    private requestData: any;

    execute(notification: puremvc.INotification) {
        const body = (this.requestData = notification.getBody() || {});
        const url = Utils.getUrl(net.HttpType.api_event_race_detail, body);
    ...
    private response(result: vo.ResponseVO) {
        if (result.status === 0) {
            ...
            this.sendNotification(net.EventType.api_event_race_detail, result.data, this.requestData.unique);
        }
    }
}
