 ...
 * 左侧菜单栏
 */
export default class cmd_api_menu_leftnav extends puremvc.SimpleCommand {
    private requestData: any;

    execute(notification: puremvc.INotification) {
        const body = (this.requestData = notification.getBody() || {});
        const url = Utils.getUrl(net.HttpType.api_menu_leftnav, body);
    ...
    private response(result: vo.ResponseVO) {
        if (result.status === 0) {
            ...
            this.sendNotification(net.EventType.api_menu_leftnav, result.data, this.requestData.unique);
        }
    }
}
