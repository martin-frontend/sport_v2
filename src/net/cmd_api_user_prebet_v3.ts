 ...
 * 预下注V3
 */
export default class cmd_api_user_prebet_v3 extends puremvc.SimpleCommand {
    private requestData: any;

    execute(notification: puremvc.INotification) {
        const body = (this.requestData = notification.getBody() || {});
        const url = Utils.getUrl(net.HttpType.api_user_prebet_v3, body);
    ...
    private response(result: vo.ResponseVO) {
        if (result.status === 0) {
            ...
            this.sendNotification(net.EventType.api_user_prebet_v3, result.data, this.requestData.unique);
        }
    }
}
