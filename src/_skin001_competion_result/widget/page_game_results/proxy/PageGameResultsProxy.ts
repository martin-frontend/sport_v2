...
export default class PageGameResultsProxy extends puremvc.Proxy {
    ...
    set_results(data: any) {
        ...
        // 全场 - 总入球, 亚洲大小盘
        this.marketTypeKind[2].result = this.marketTypeKind[3].result =
            parseInt(data.goals_ft.split("-")[0]) + parseInt(data.goals_ft.split("-")[1]);
        // 全场 - 亚洲大小盘 - 角球
        ...
        // 两队都得分
        this.marketTypeKind[7].result =
            data.goals_ft.split("-")[0] > 0 && data.goals_ft.split("-")[1] > 0 ? LangUtil("是") : LangUtil("否");
        ...
        // 主队零失球获胜
        this.marketTypeKind[12].result =
            data.goals_ft.split("-")[0] > data.goals_ft.split("-")[1] && data.goals_ft.split("-")[1] == 0 ? LangUtil("是") : LangUtil("否");
        // 客队零失球获胜
        this.marketTypeKind[13].result =
            data.goals_ft.split("-")[0] < data.goals_ft.split("-")[1] && data.goals_ft.split("-")[0] == 0 ? LangUtil("是") : LangUtil("否");
        // 入球单双
        this.marketTypeKind[14].result =
            (parseInt(data.goals_ft.split("-")[0]) + parseInt(data.goals_ft.split("-")[1])) % 2 == 0 ? LangUtil("双") : LangUtil("单");
        // 半场 - 波胆
        this.marketTypeKind[15].result = this.marketTypeKind[24].result = data.goals_ht;
        // 半场 - 亚洲大小盘, 总入球
        this.marketTypeKind[17].result = this.marketTypeKind[18].result =
            parseInt(data.goals_ht.split("-")[0]) + parseInt(data.goals_ht.split("-")[1]);
        // 半场 - 主队零失球获胜
        this.marketTypeKind[20].result =
            data.goals_ht.split("-")[0] > data.goals_ht.split("-")[1] && data.goals_ht.split("-")[1] == 0 ? LangUtil("是") : LangUtil("否");
        // 半场 - 客队零失球获胜
        this.marketTypeKind[21].result =
            data.goals_ht.split("-")[0] < data.goals_ht.split("-")[1] && data.goals_ht.split("-")[0] == 0 ? LangUtil("是") : LangUtil("否");
        // 半场 - 单/双
        this.marketTypeKind[22].result =
            (parseInt(data.goals_ht.split("-")[0]) + parseInt(data.goals_ht.split("-")[1])) % 2 == 0 ? LangUtil("双") : LangUtil("单");
        //半场 - 两队都得分
        this.marketTypeKind[23].result =
            data.goals_ht.split("-")[0] > 0 && data.goals_ht.split("-")[1] > 0 ? LangUtil("是") : LangUtil("否");
        // 亚洲让球盘 - 全场加时
